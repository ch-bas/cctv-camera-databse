# Ring Video Doorbell 4

*Also known as: B08JNR77QY*

| Field | Spec |
|-------|------|
| Brand | Ring |
| Model | Video Doorbell 4 |
| Type | doorbell |
| Connectivity | wifi |
| Resolution | 1080p (2MP, 1920×1080) |
| Lens | 1× |
| Field of view | 155 diagonal° |
| Night vision | color (5m) |
| Power | Quick-Release rechargeable battery / hardwired 8–24 VAC |
| Protocols | http |
| IP rating | IPX5 |
| Two-way audio | Yes |
| Operating temp | -20 to 48°C |
| Released | 2021 |

## Features

- color Pre-Roll (4 sec before event)
- advanced motion detection
- customizable privacy zones
- quick replies
- dual-band WiFi 2.4/5 GHz
- Alexa and Google Assistant integration
- Ring subscription required for cloud video history

## Sources

- https://ring.com/gb/en/support/articles/uc381/Learn-About-Video-Doorbell-4

---
*Auto-generated from ring-video-doorbell-4.json — do not edit by hand.*
